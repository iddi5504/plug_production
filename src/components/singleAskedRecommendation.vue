    ...
    <div :key="askedRecommendation.Id" class="recommendmepostcontainer">
        <div class="recommendmecontent">
            ...
            <div style="display: flex; flex-direction: column; align-items: center;">
                <img v-if="askedRecommendation.type === 'Game'" :ref="askedRecommendation.Id" class="recommendmeimage"
                    src="../assets/game.jpg" alt="">
                <img v-if="askedRecommendation.type === 'Music'" :ref="askedRecommendation.Id" class="recommendmeimage"
                    src="../assets/music.jpg" alt="">
                <img v-if="askedRecommendation.type === 'Book'" :ref="askedRecommendation.Id" class="recommendmeimage"
                    src="../assets/book.jpg" alt="">
                <img v-if="askedRecommendation.type === 'Movie'" :ref="askedRecommendation.Id" class="recommendmeimage"
                    src="../assets/movie.jpg" alt="">
            ...
            <div>
                <userRecommendation v-for="recommendation in recommendations" :recommendation="recommendation"
                    :askedRecommendation="askedRecommendation" :key="recommendation.id">
                </userRecommendation>
                <div class="no-recommendation" v-show="(recommendations.length == 0)">
                    <div>No recommendations made yet</div>
                </div>
                <div>
                </div>
...
<script>
import { addDoc, arrayUnion, collection, deleteDoc, doc, getDoc, getDocs, increment, limit, query, serverTimestamp, setDoc, updateDoc, where } from '@firebase/firestore';
...
export default {
    ...
    methods: {
        recommend() {
            ...
            const userRecommendationsCollection = collection(firestore, 'userRecommendations')
            if (this.showRecommendinput) {
                if (this.recommendationTitle.length > 0) {
                    const recommendation = doc(userRecommendationsCollection)
                    recommendationData.recommendationId = recommendation.id
                    setDoc(recommendation, recommendationData)
                        .then(() => {
                            this.showRecommendinput = false
                            this.recommendationTitle = ''
                            this.recommendationDescription = ''
                            this.showRecommendSucess = true
                            this.recommendations.unshift(recommendationData)
                            setTimeout(() => {
        ...
        async getRecommendations() {
            ...
            const userRecommendationsCollection = collection(firestore, 'userRecommendations')
            const q = query(userRecommendationsCollection, where('askedRecommendationId', '==', this.askedRecommendation.Id), limit(3))
            const userRecommendations = await getDocs(q)
            userRecommendations.forEach((snapshot) => {
                this.recommendations.push(snapshot.data())
            })
    ...
    async created() {
        // get askedRecommendation
        const askedRecommendationId = this.$route.params.id
        const askedRecommendationCollection = query(collection(firestore, 'AskedRecommendations'), where('Id', '==', askedRecommendationId))
        const askedRecommendations = await getDocs(askedRecommendationCollection)
            .catch(() => {
                this.errorOccurred = true;
                this.errorMessage = 'Could not fetch askedRecommendations'
            })
        this.askedRecommendation = { ...askedRecommendations.docs[0].data() }

        // delete userRecommendations
        bus.$on('deleteUserRecommendation', (index) => {
            this.recommendations.splice(index, 1)
            this.askedRecommendation.number_of_recommendations--
            const askedRecommendationDoc = doc(firestore, `/AskedRecommendations/${this.askedRecommendation.Id}`)
            updateDoc(askedRecommendationDoc, {
                number_of_recommendations: increment(-1)
            })
        })
