    ...
    getters: {
        FEED(state) {
            ...
            if (state.selectedCategory == 'all') {
                let allLazyLoadedFeed = [...state.allLazyLoadedFeed]
                allLazyLoadedFeed.forEach(loadedData => {
                    state.allFeed.push(loadedData)
                })
                state.allLazyLoadedFeed = []
    ...
    actions: {
        async getRecommendations(context) {
            ...
            BooksDocs.forEach(snapshot => {
                bookRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
            })
            MoviesDocs.forEach(snapshot => {
                movieRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
            })
            GamesDocs.forEach(snapshot => {
                gameRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
            })
            MusicsDocs.forEach(snapshot => {
                musicRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
            })
        ...
        async getPosts(context) {
            const MusicsPostsQuery = query(collectionGroup(firestore, 'MusicPosts'), orderBy('upvotes', "desc"),  startAfter(context.state.MusicPostStartingDoc))
            const MoviesPostsQuery = query(collectionGroup(firestore, 'MoviePosts'), orderBy('upvotes', "desc"),  startAfter(context.state.MoviePostStartingDoc))
            const GamesPostsQuery = query(collectionGroup(firestore, 'GamePosts'), orderBy('upvotes', "desc"),  startAfter(context.state.GamePostStartingDoc))
            const BooksPostsQuery = query(collectionGroup(firestore, 'BookPosts'), orderBy('upvotes', "desc"),  startAfter(context.state.BookPostStartingDoc))
        ...
        async lazyLoadPosts(context) {
            ...
            switch (category) {
                case 'all':
                    const allMusicsQuery = query(collectionGroup(firestore, 'MusicRecommendations'), orderBy('upvotes', "desc"),  startAfter(context.state.MusicRecommendationStartingDoc))
                    const allMoviesQuery = query(collectionGroup(firestore, 'MovieRecommendations'), orderBy('upvotes', "desc"),  startAfter(context.state.MovieRecommendationStartingDoc))
                    const allGamesQuery = query(collectionGroup(firestore, 'GameRecommendations'), orderBy('upvotes', "desc"),  startAfter(context.state.GameRecommendationStartingDoc))
                    const allBooksQuery = query(collectionGroup(firestore, 'BookRecommendations'), orderBy('upvotes', "desc"),  startAfter(context.state.BookRecommendationStartingDoc))
                    ...
                    allBooksDocs.forEach(snapshot => {
                        allLazyLoadedBookRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                    allMoviesDocs.forEach(snapshot => {
                        allLazyLoadedMovieRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                    allGamesDocs.forEach(snapshot => {
                        allLazyLoadedGameRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                    allMusicsDocs.forEach(snapshot => {
                        allLazyLoadedMusicRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                ...
                case 'Musics':
                    ...
                    context.state.MusicRecommendationStartingDoc = MusicsDocs.docs[MusicsDocs.docs.length - 1]
                    MusicsDocs.forEach(snapshot => {
                        lazyLoadedMusicRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                ...
                case 'Movies':
                    ...
                    context.state.MovieRecommendationStartingDoc = MoviesDocs.docs[MoviesDocs.docs.length - 1]
                    MoviesDocs.forEach(snapshot => {
                        lazyLoadedMovieRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                ...
                case 'Books':
                    ...
                    context.state.BookRecommendationStartingDoc = BooksDocs.docs[BooksDocs.docs.length - 1]
                    BooksDocs.forEach(snapshot => {
                        lazyLoadedBookRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
                ...
                case 'Games':
                    ...
                    context.state.GameRecommendationStartingDoc = GamesDocs.docs[GamesDocs.docs.length - 1]
                    GamesDocs.forEach(snapshot => {
                        lazyLoadedGameRecommendations.push({ ...snapshot.data(), type: 'recommendation', uid:snapshot.id })
                    })
